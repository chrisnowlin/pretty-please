...
## Overview
REST API endpoints to expose `EmbeddingConfig` and `OCRConfig` capabilities to frontend clients.
...
**Models** (in `src/jina_rag_pipeline/api/models.py`):
- `EmbeddingConfigModel` (Pydantic)
- `OCRConfigModel` (Pydantic)
- `PresetInfo` (Pydantic)

**Endpoints** (in `src/jina_rag_pipeline/api/app.py`):
...
**Persistence** (in `src/jina_rag_pipeline/api/config_manager.py`):
- `ConfigManager` class for reading/writing `data/config.json`
- Thread-safe updates
